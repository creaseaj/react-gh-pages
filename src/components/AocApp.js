...
const ShowList = (props) => {
    let output = [];
    for (let i = 1; i <= 25; i++) {
        output.push(<div className="text-lg hover:bg-gray-700 hover:text-white" id={i} key={i}
            onClick={(e) => {
...
const AocApp = () => {
    ...
    return (
        <div className=" w-screen h-screen flex items-center sm:-mt-16">
            {showSelectedDay ? (
                <div className=" z-20 absolute right-0 bottom-0 top-0 left-0" onClick={() => setShowSelectedDay(!showSelectedDay)} />
            ) : null}
            <div className="relative flex sm:container sm:mx-auto w-screen h-screen sm:h-app bg-gray-200 sm:rounded-xl p-10 justify-end">
                ...
                {showSelectedDay ? (
                    <div className=" z-30 absolute right-0 bottom-0  border-2 rounded-xl border-gray-700 bg-gray-100 w-72 divide-y ">
